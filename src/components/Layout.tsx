...
function Logo() {
  return (
    <Link to="/" className="flex items-center gap-3 shrink-0 no-underline">
      <div className="relative flex items-center justify-center w-10 h-10 shrink-0" style={{ background: NAVY }}>
        <span className="font-display font-800 text-white text-[13px] tracking-widest leading-none">NB</span>
        <div className="absolute bottom-0 left-0 right-0 h-[3px]" style={{ background: GOLD }} />
      </div>
      <div>
        <div className="font-display font-800 text-[17px] tracking-[0.1em] uppercase leading-none" style={{ color: NAVY }}>NB LAO</div>
        <div className="font-ui text-[8px] tracking-[0.22em] uppercase mt-[3px]" style={{ color: '#9aabbd' }}>Sole Co., Ltd.</div>
      </div>
    </Link>
  )
}
...
// ─── Desktop Mega Menu ────────────────────────────────────────────────────────
function MegaMenu({ onClose, t }: { onClose: () => void; t: ReturnType<typeof useT> }) {
  return (
    <div className="bg-white w-full" style={{ border: '1px solid #dde2ea', borderTop: `2px solid ${GOLD}`, boxShadow: '0 8px 32px rgba(0,0,0,0.1)' }}>
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-6">
        <div className="flex items-center justify-between mb-5">
          <span className="font-display font-700 text-[13px] tracking-wide" style={{ color: NAVY }}>{t.header.browse_category}</span>
          <Link to="/products" onClick={onClose} className="font-ui text-[12px] no-underline flex items-center gap-1 hover:underline" style={{ color: BLUE }}>
...
function LangSwitcherHeader() {
  ...
  return (
    <div className="flex items-center shrink-0" style={{ border: '1px solid #dde2ea', borderRadius: 3 }}>
      <button
        onClick={() => setLang('en')}
...
function LangSwitcherUtility() {
  const { lang, setLang } = useStore()
  return (
    <div className="flex items-center gap-0 shrink-0" style={{ border: '1px solid rgba(255,255,255,0.18)', borderRadius: 3 }}>
      <button
        onClick={() => setLang('en')}
...
function MobileDrawer({ t }: { t: ReturnType<typeof useT> }) {
  ...
  const navLinks = NAV_ITEMS.filter(i => i.labelKey !== 'home' && i.labelKey !== 'products')
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid #e4e8ef' }}>
          <Logo />
          <button onClick={onClose} className="p-1" style={{ color: NAVY }}><XIcon /></button>
        </div>
...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50" style={{ boxShadow: '0 1px 0 #dde2ea' }}>
      ...
      <div className="hidden lg:block" style={{ background: NAVY }}>
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-[7px] flex items-center justify-between">
          <div className="flex items-center gap-5">
            <a href="tel:+85620555567890" className="flex items-center gap-1.5 font-ui text-[11px]" style={{ color: 'rgba(255,255,255,0.55)' }}>
              <PhoneIcon /> +856 20 5555 6789
            </a>
            <a href="mailto:info@nblaosolelao.com" className="flex items-center gap-1.5 font-ui text-[11px]" style={{ color: 'rgba(255,255,255,0.55)' }}>
              <MailIcon /> info@nblaosolelao.com
            </a>
          </div>
          <div className="flex items-center gap-4">
            <span className="font-ui text-[10px] tracking-[0.16em] uppercase" style={{ color: 'rgba(255,255,255,0.35)' }}>
              Industrial Supply · Vientiane, Laos PDR
            </span>
            <Link to="/admin" className="font-ui text-[10px] tracking-wide" style={{ color: 'rgba(255,255,255,0.35)' }}>
              {t.nav.admin}
            </Link>
          </div>
        </div>
      ...
      {/* ── Main header ──────────────────────────────────────────────────────── */}
      <div className="bg-white relative" style={{ borderBottom: '1px solid #eaecf0' }}>
          ...
          <div className="flex items-center justify-between h-[62px] gap-3 lg:gap-6">
            ...
            <div className="hidden lg:flex items-center gap-3 shrink-0">
              ...
              <form onSubmit={handleSearch} className="shrink-0">
                <div className="flex items-center gap-2 px-3 py-1.5 w-[240px]"
                  style={{ border: '1px solid #dde2ea', background: '#f7f8fa' }}>
                  <span className="text-[#94a3b8] shrink-0"><SearchIcon /></span>
                  <input
                    value={search}
              ...
              <Link to="/account" className="text-[#94a3b8] hover:text-[#082E61] transition-colors">
                <UserIcon />
              </Link>
              <Link to="/cart" className="relative text-[#94a3b8] hover:text-[#082E61] transition-colors">
                <CartIcon />
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 flex items-center justify-center text-white text-[9px] font-700 rounded-full" style={{ background: BLUE }}>
            ...
            <div className="lg:hidden flex items-center gap-3">
              <Link to="/account" style={{ color: '#94a3b8' }}><UserIcon /></Link>
              <Link to="/cart" className="relative" style={{ color: '#94a3b8' }}>
                <CartIcon />
                {cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 w-4 h-4 flex items-center justify-center text-white text-[9px] font-700 rounded-full" style={{ background: BLUE }}>
                    {cartCount > 9 ? '9+' : cartCount}
              ...
              <button style={{ color: NAVY }} onClick={() => setActiveDrawer('nav')}>
                <MenuIcon />
              </button>
            </div>
          </div>
...
function FooterSection({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }} className="md:border-none">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between py-3 md:hidden"
        style={{ background: 'transparent', color: '#fff' }}
      >
        <span className="font-display font-700 text-[11px] tracking-[0.18em] uppercase">{title}</span>
        <span style={{ transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
          <ChevronDown size={14} />
        </span>
      </button>
      <h4 className="hidden md:block font-display font-700 text-[11px] tracking-[0.18em] uppercase mb-3 text-white">{title}</h4>
...
export function Footer() {
  ...
  return (
    <footer style={{ background: '#060f1c', color: '#7a8ea6' }}>
      <div className="h-[2px]" style={{ background: `linear-gradient(90deg, ${GOLD} 0%, #b8941e 30%, transparent 100%)` }} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-[220px_1fr_1fr_1fr] gap-0 md:gap-8 lg:gap-10">
          ...
          <div className="mb-6 md:mb-0">
            <div className="flex items-center gap-3 mb-4">
              <div className="relative flex items-center justify-center w-9 h-9" style={{ background: NAVY }}>
                <span className="font-display font-800 text-white text-[12px] tracking-widest">NB</span>
                <div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ background: GOLD }} />
              </div>
              <div>
                <div className="font-display font-800 text-[15px] tracking-[0.1em] uppercase text-white">NB LAO</div>
                <div className="font-ui text-[8px] tracking-[0.2em] uppercase mt-0.5" style={{ color: '#3d5168' }}>Sole Co., Ltd.</div>
              </div>
            </div>
            <p className="font-ui text-[12px] leading-[1.7] mb-4">{t.footer.tagline}</p>
            <div className="flex flex-col gap-2">
              <span className="flex items-start gap-2 font-ui text-[12px]">
                <span className="mt-0.5 shrink-0" style={{ color: GOLD }}><MapPinIcon /></span>
                123 Industrial Zone, Saysettha, Vientiane, Laos PDR
              </span>
              <a href="tel:+85620555567890" className="flex items-center gap-2 font-ui text-[12px] hover:text-white transition-colors">
                <span style={{ color: GOLD }}><PhoneIcon /></span> +856 20 5555 6789
              </a>
              <a href="mailto:info@nblaosolelao.com" className="flex items-center gap-2 font-ui text-[12px] hover:text-white transition-colors">
                <span style={{ color: GOLD }}><MailIcon /></span> info@nblaosolelao.com
              </a>
            </div>
          </div>
          ...
          <FooterSection title={t.footer.products}>
            <ul className="flex flex-col gap-1.5">
              {['Electrical Equipment', 'Industrial Machinery', 'Power & Energy', 'Tools & Equipment', 'Safety & Fire Protection', 'Cables & Accessories'].map((item) => (
                <li key={item}>
                  <Link to={`/products?category=${encodeURIComponent(item)}`} className="font-ui text-[12px] hover:text-white transition-colors no-underline">{item}</Link>
                </li>
              ))}
            </ul>
          </FooterSection>
          ...
          <FooterSection title={t.footer.company}>
            <ul className="flex flex-col gap-1.5">
              {[
                ...
                <li key={item.label}>
                  <Link to={item.to} className="font-ui text-[12px] hover:text-white transition-colors no-underline">{item.label}</Link>
                </li>
              ))}
            </ul>
          </FooterSection>
          ...
          <FooterSection title={t.footer.quick_access}>
            <ul className="flex flex-col gap-1.5">
              {[
                ...
                { label: t.footer.my_quotations, to: '/account?tab=quotations' },
      ...
      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="max-w-[1440px] mx-auto px-5 sm:px-6 lg:px-12 py-3.5 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="font-ui text-[11px]">© {new Date().getFullYear()} NB Lao Sole Co., Ltd. {t.footer.rights}</p>
          <div className="flex gap-5">
            <Link to="/privacy" className="font-ui text-[11px] hover:text-white transition-colors no-underline">{t.footer.privacy}</Link>
            <Link to="/terms" className="font-ui text-[11px] hover:text-white transition-colors no-underline">{t.footer.terms}</Link>
          </div>
        </div>
      </div>
    </footer>
...
export function PageLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex flex-col">
      ...
      */}
      <main className="flex-1 pt-[114px] lg:pt-[96px]">
        {children}
      </main>
      <Footer />
    </div>
